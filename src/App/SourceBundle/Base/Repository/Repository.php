...
abstract class Repository extends Core {
	...
	public function collect(array $filters = [], array $paging = [], array $settings = [])
	{
        ...
        $totalSelect = clone $qb;

        if ( Arr::get($settings, 'get_total'))
        {
	        $total = $totalSelect->select('entity.id')->getQuery()->getScalarResult();
	        $total = count($total);
        }
		...
		return Arr::get($settings, 'get_total') || Arr::get($settings, 'get_rows') ? [ 'rows' => $result, 'total' => $total ] : $result;
	}
