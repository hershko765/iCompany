...
class Get extends HandlerManager implements Handler {
	...
	{
		$Employee = $this->em->getRepository('AppManagerBundle:Model\Employee\Permission')->find($this->id);

		if ( ! $Employee)
			throw new NotFoundHttpException('Employee Permission #'.$this->id.' Not Found');
