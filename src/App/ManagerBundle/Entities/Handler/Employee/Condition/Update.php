...
class Update extends HandlerManager implements Handler {
	...
	protected function _execute()
	{
		...
		$employeeCondition = $repo->find($this->id);
		if ( ! $employeeCondition)
			throw new NotFoundHttpException('Employee Condition ID #'.$this->id.' Not Found');

		$repo->hydrate($this->data, $employeeCondition, Repository::PERM_UPDATE);

		// Validate model, if errors found return them
		$errors = $this->validate->validate($employeeCondition);
		...
		return [
			'status' => TRUE,
			'data_array' => $repo->save($employeeCondition)
		];
	}
}