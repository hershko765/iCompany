...
class Get extends HandlerManager implements Handler {
	...
	{
		$Employee = $this->em->getRepository('AppManagerBundle:Model\Employee\Condition')->find($this->id);

		if ( ! $Employee)
			throw new NotFoundHttpException('Employee Condition #'.$this->id.' Not Found');
